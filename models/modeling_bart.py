...
from .sparselinear import (
    SparseLinear,
    MaskedMLP,
    MaskedElementWiseVector,
    MaskedElementWiseVectorSTE,
    MaskedElementWiseVectorScalar,
)
...
@add_start_docstrings(
    "Bart model for sequence classification. ", BART_START_DOCSTRING
)
...
class BartForTokenAttentionCLS(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        # encoder_outputs["last_hidden_state"]: (batch_size, sequence_length, hidden_size)
        #print(encoder_outputs["last_hidden_state"].shape, self.learnable_mask.shape)
        bsz, seq_len, hidden_size = encoder_outputs["last_hidden_state"].shape
        attn_weights = torch.bmm(encoder_outputs["last_hidden_state"], self.token_query.unsqueeze(0).expand(bsz, self.config.hidden_size, 1))
        attn_weights = nn.functional.softmax(attn_weights, dim=1)
...
class BartForDisentangledRepresentation(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        # encoder_outputs["last_hidden_state"]: (batch_size, sequence_length, hidden_size)
        #print(encoder_outputs["last_hidden_state"].shape, self.learnable_mask.shape)
        #masked_embeddings = encoder_outputs["last_hidden_state"]*self.sharp_mask_activate(self.learnable_mask)
        masked_embeddings, mask_weight = self.learnable_mask(encoder_outputs["last_hidden_state"])
        # masked_embeddings: (batch_size, sequence_length, hidden_size)
...
class BartForTokenAttentionSparseCLS(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        attn_weights = nn.functional.softmax(attn_weights, dim=1)
        masked_embeddings, mask_weight = self.learnable_mask(encoder_outputs["last_hidden_state"])
        # attn_weights = torch.bmm(encoder_outputs["last_hidden_state"], mask_weight.unsqueeze(1).unsqueeze(0).expand(bsz, self.config.hidden_size, 1))
        # attn_weights = nn.functional.softmax(attn_weights, dim=1)
...
class BartForTokenAttentionSparseCLSJoint(BartPretrainedModel):
    base_model_prefix = "model"
...
class BartForTokenAttentionSparseCLSJoint_1minusMask(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: BartConfig):
        ...
        self.freeze_params(self.model)
        self.learnable_mask = MaskedElementWiseVector(config.hidden_size)
        self.freeze_params(self.learnable_mask)
        #self.token_query = nn.Parameter(torch.randn(config.hidden_size, 1))
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.alpha = 0.0005 # scale for regularization of mask layer
        # Initialize weights and apply final processing
    ...
    def forward(
        ...
        bsz, seq_len, hidden_size = encoder_outputs["last_hidden_state"].shape
        masked_embeddings, mask_weight = self.learnable_mask(encoder_outputs["last_hidden_state"])
        mask_weight = 1 - mask_weight
        masked_embeddings = encoder_outputs["last_hidden_state"] * mask_weight
        attn_weights = torch.bmm(masked_embeddings,
                                 mask_weight.unsqueeze(1).unsqueeze(0).expand(bsz, self.config.hidden_size, 1))
        attn_weights = nn.functional.softmax(attn_weights, dim=1)
        masked_embeddings = masked_embeddings * attn_weights.expand(bsz, seq_len, hidden_size)
...
class BartForTokenAttentionSparseCLSJointSTE(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.model = BartModel(config)
        self.freeze_params(self.model)
        self.learnable_mask = MaskedElementWiseVectorSTE(config.hidden_size)
        #self.sharp_mask_activate = nn.Sigmoid()
        #self.token_query = nn.Parameter(torch.randn(config.hidden_size, 1))
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.alpha = 0.0005 # scale for regularization of mask layer
...
class BartForTokenAttentionSparseCLSJointScalar(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: BartConfig):
        ...
        self.model = BartModel(config)
        self.freeze_params(self.model)
        self.learnable_mask = MaskedElementWiseVectorScalar(config.hidden_size)
        #self.sharp_mask_activate = nn.Sigmoid()
        #self.token_query = nn.Parameter(torch.randn(config.hidden_size, 1))
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.alpha = 0.0005 # scale for regularization of mask layer
...
def initialize_sInit():
    return 0.05*torch.ones([1, 1])

class BartForTokenAttentionSparseCLSJointSTR(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        bsz, seq_len, hidden_size = encoder_outputs["last_hidden_state"].shape
        masked_embeddings = sparseFunction(encoder_outputs["last_hidden_state"], self.sparseThreshold, self.activation, self.f)
        print(torch.sum(masked_embeddings == 0.) / masked_embeddings.numel())
        merged_label_vector = torch.sum(masked_embeddings, dim=1)
        logits = self.classifier(merged_label_vector)
        loss = None
        if labels is not None:
...
class BartForSequenceClassificationMultiClass(BartPretrainedModel):
    base_model_prefix = "model"
...
class BartForTokenAttentionCLSMultiClass(BartPretrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: BartConfig):
        ...
        self.beta = 0.5 # scale for regularization of distance between mask layers
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        for attn, convert_vector in zip(self.attn_vectors, self.convert2scalar_vectors):
            attn_weights = torch.bmm(encoder_outputs["last_hidden_state"], attn.unsqueeze(0).expand(bsz, self.config.hidden_size, 1))
            attn_weights = nn.functional.softmax(attn_weights, dim=1)
            attn_embeddings = encoder_outputs["last_hidden_state"] * attn_weights.expand(bsz, seq_len, hidden_size)
            merged_label_vector = torch.sum(attn_embeddings, dim=1)
...
class BartForDisentangledRepresentationMultiClass(BartPretrainedModel):
    base_model_prefix = "model"
...
class BartForTokenAttentionSparseCLSJointMultiClass(BartPretrainedModel):
    base_model_prefix = "model"
