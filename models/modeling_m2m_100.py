...
class DisentangledSequenceClassifierOutput(ModelOutput):
    ...
    disentangle_mask: Optional[Tuple[torch.FloatTensor]] = None
    disentangled_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    token_attention_vector: Optional[Tuple[torch.FloatTensor]] = None
...
@add_start_docstrings(
    "M2M100 model for disentangled representation learning. ", M2M_100_START_DOCSTRING
)
class M2M100ForDisentangledRepresentation(M2M100PreTrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"encoder.version",
        r"decoder.version",
...
@add_start_docstrings(
    "M2M100 model for sparsity representation learning. ", M2M_100_START_DOCSTRING
)
class M2M100ForSparsityMatrix(M2M100PreTrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"encoder.version",
        r"decoder.version",
    ...
    def __init__(self, config: M2M100Config):
        ...
        self.model = M2M100Model(config)
        self.freeze_params(self.model)
        connectivity = torch.tensor([range(1024),range(1024)])
        #self.learnable_matrix = SparseLinear(config.hidden_size, config.hidden_size, bias=False, connectivity=connectivity, dynamic=True)
        self.learnable_matrix = MaskedMLP(config.hidden_size, config.hidden_size)
        #self.matrix_activate = nn.Sigmoid()
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.alpha = 0.001 # scale for L1 norm of learnable mask
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        # encoder_outputs["last_hidden_state"]: (batch_size, sequence_length, hidden_size)
        #print(encoder_outputs["last_hidden_state"].shape, self.learnable_mask.shape)
        masked_embeddings = self.learnable_matrix(encoder_outputs["last_hidden_state"])
        #masked_embeddings = self.matrix_activate(masked_embeddings)
        # masked_embeddings: (batch_size, sequence_length, hidden_size)
        #print(masked_embeddings.shape)
        merged_label_vector = torch.sum(masked_embeddings, dim=1)
        #print(merged_label_vector.shape)
        logits = self.classifier(merged_label_vector)
        #print(self.learnable_matrix.weight)

        loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) #+ self.alpha*torch.linalg.norm(self.learnable_mask, 1)
            #print(loss, type(loss))
        ...
        return DisentangledSequenceClassifierOutput(
            ...
            logits=logits,
            hidden_states=encoder_outputs.last_hidden_state,
            disentangle_mask=self.learnable_matrix.weight,
            disentangled_hidden_states=masked_embeddings,
            attentions=encoder_outputs.attentions,
        )
...
@add_start_docstrings(
    "M2M100 model for sequence classification. ", M2M_100_START_DOCSTRING
)
class M2M100ForSequenceClassification(M2M100PreTrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [
        r"encoder.version",
        r"decoder.version",
