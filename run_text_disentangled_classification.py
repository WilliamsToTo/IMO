        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            if num_labels > 2:
                ...
                all_eval_metric = [accuracy_score(y_true=all_references, y_pred=all_predictions),
                               precision_score(y_true=all_references, y_pred=all_predictions, average="micro",
                                               zero_division=1),
                               recall_score(y_true=all_references, y_pred=all_predictions, average="micro",
                                            zero_division=1),
                               f1_score(y_true=all_references, y_pred=all_predictions, average="micro")]
            else:
                # eval_metric = [metric.compute(), p_metric.compute(), r_metric.compute(), f1_metric.compute()]
                all_eval_metric = [accuracy_score(y_true=all_references, y_pred=all_predictions),
                               precision_score(y_true=all_references, y_pred=all_predictions),
